...
jQuery(document).ready(function ($) {
	...
	jQuery(document).on('click','.get-detail',function(){
		jQuery(this).parents('tr').next('tr').next('tr').hide();
		jQuery(this).parents('tr').next('tr').slideToggle(200);
	});

	// Edit Appoinment
	jQuery(document).on('click','.edit-appoinment',function(){
		jQuery(this).parents('tr').next('tr').hide();
		jQuery(this).parents('tr').next('tr').next('tr').slideToggle(200);
	});
						...
						jQuery.ajax({
							...
							success: function(response) {
								...
								if( response.action_type == 'approved' ){
									...
									_this.parents('td').html(approved({id: id}));
