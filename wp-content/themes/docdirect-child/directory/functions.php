...
if ( ! function_exists( 'docdirect_account_settings' ) ) {
    ...
    function sendEmailForKeyword($to, $from, $subject, $message) {
        $headers = "From: ". $from . "\r\nReply-To: ".$from."\r\n";
	...
	function docdirect_account_settings(){ 
        ...
        if(($_POST["user-new-tags-data"]))  {
            ...
            if(count($allNewAddedIds)>0) {
                ...
                $message_content = $current_user->nickname." kullanıcısı ".implode(",",$all_added_tags)." etiket(ler)ini ekledi. Toplam yeni etiket : ".count($allNewAddedIds);
                $result = sendEmailForKeyword($to, $from, $subject, $message_content);
            }
        }
