...
<div class="doc-booking-listings dr-bookings">
  <div class="tg-dashboard tg-docappointmentlisting tg-haslayout">
    ...
    <div class="tg-appointmenttable">
      <table class="table">
        <thead class="thead-inverse">
          <tr>
            ...
            <th style="padding: 0px; vertical-align: middle;">
              <button id="new-appoinment" class="btn btn-primary btn-sm"><?php esc_html_e('New Appointment','docdirect');?></button>
            </th>
          </tr>
        </thead>
        <tbody>
          <tr id="new-appoinment-form" class="tg-appointmentdetail bk-elm-hide">
            <td colspan="6" style="background-color: #f7f7f7 !important;">
              <form style="position: relative;" action="#" method="post" id="appoinment-form-0">
                <div class="appointment-data-wrap">
                  <div class="form-group">
                    <div class="doc-select">
                      <select name="bk_category" class="bk_category_update" data-id="0">
                  ...
                  <div class="form-group">
                    <div class="doc-select">
                      <select name="bk_service" id="bk_service0">
                        <option value=""><?php esc_html_e('Select Service*','docdirect');?></option>
                        <?php 
                        if( !empty($booking_all_services) && !empty($booking_all_services[$bk_category])) {
                          foreach($booking_all_services[$bk_category] as $key => $value) {
                        ?>
                  ...
                  <div class="bk-booking-schedules">
                    <div class="tg-appointmenttime" style="position: relative; padding: 10px;">
                      <div id="picker0" class="tg-dayname booking-pickr"> 
                        <strong><?php echo date_i18n($date_format, strtotime($current_date)); ?></strong>
                      ...
                      <div class="tg-timeslots step-two-slots">
                        <div class="tg-timeslotswrapper">
                          <?php echo docdirect_prepare_booking_step_two($user_identity, $current_date, '0'); ?>
                        </div>
                      </div>
                    </div>
                  </div>
                  <script type="text/javascript">
                    jQuery(function() {
                      update_calendar('0', '<?php echo $current_date; ?>');
                    });
                  </script>
                  <div class="form-group">
                  ...
                  <div class="form-group">
                    <input type="text" class="form-control" name="username" placeholder="<?php esc_attr_e('Your Name*','docdirect');?>">
                  </div>
                  <div class="form-group">
                    <input type="text" class="form-control" name="userphone" placeholder="<?php esc_attr_e('Phone*','docdirect');?>">
                  </div>
                  <div class="form-group">
                    <input type="text" class="form-control" name="useremail" placeholder="<?php esc_attr_e('Email*','docdirect');?>">
                  </div>
                  ...
                  <div class="form-group tg-textarea">
                    ...
                    <button type="button" class="btn btn-primary save-appoinment" data-id="0"><?php esc_html_e('Save','docdirect');?></button>
                  </div>
                </div>
              </form>
            </td>
          ...
          $query = new WP_Query($args);
            ...
            while($query->have_posts()) : $query->the_post();
    		  ...
    		  ?>
          <tr class="<?php echo esc_attr( $trClass );?> booking-<?php echo intval( $post_id );?>">
            ...
            <td>
              ...
              <?php }else {?>
              <a class="tg-btncheck get-process" data-type="approve" data-id="<?php echo intval( $post_id );?>" href="javascript:;"><?php esc_html_e('Approve','docdirect');?></a> 
              <?php }?>
              <a class="tg-btnclose get-process" data-type="cancel" data-id="<?php echo intval( $post_id );?>" href="javascript:;"><?php esc_html_e('Cancel','docdirect');?></a>
              <a class="tg-btnedit edit-appoinment" data-id="<?php echo intval( $post_id );?>" href="javascript:;"><?php esc_html_e('Edit','docdirect');?></a>
            </td>
          </tr>
            ...
            <td colspan="6" style="background-color: #f7f7f7 !important;">
              <form style="position: relative;" action="#" method="post" id="appoinment-form-<?php echo intval( $post_id );?>">
                <div class="appointment-data-wrap">
                  <div class="form-group">
                    <div class="doc-select">
                      <select name="bk_category" class="bk_category_update" data-id="<?php echo intval($post_id); ?>">
                        <option value=""><?php esc_html_e('Select Category*','docdirect');?></option>
                        <?php 
                        if( !empty( $services_cats ) ) {
                          foreach( $services_cats as $key => $value ) {
                  ...
                  <div class="bk-booking-schedules">
                    <div class="tg-appointmenttime" style="position: relative; padding: 10px;">
                      ...
                      <div class="tg-timeslots step-two-slots">
                        <div class="tg-timeslotswrapper">
                          <?php echo docdirect_prepare_booking_step_two($user_identity, $bk_booking_date, $post_id); ?>
                        </div>
                      </div>
                    </div>
                  </div>
                  <script type="text/javascript">
                    jQuery(function() {
                      ...
                      update_calendar(post_id, post_date);
                    });
                  </script>
                  <div class="form-group">
      ...
      <script>
        var services = jQuery.parseJSON('<?php echo addslashes(json_encode($booking_services));?>');
        ...
        jQuery(document).on('change', '.bk_category_update', function (event) {
          ...
          var selected_services = category_services[category_id] ? category_services[category_id] : services;
        ...
        function update_calendar(post_id, post_date) {
          ...
          picker.datetimepicker({
            ...
            onChangeDateTime: function(dp, $input) {
              ...
              var data_id = "<?php echo intval( $user_identity ); ?>";
              var data_string = 'post_id='+post_id+'&slot_date='+slot_date+'&data_id='+data_id+'&action=docdirect_get_booking_step_two';
              jQuery.ajax({
                type: "POST",
