 ...
 * @return {}
 */
if ( ! function_exists( 'docdirect_update_booking' ) ) {
	function docdirect_update_booking(){ 
		global $current_user;
		$user_identity = $current_user->ID;
		...
		$post_id = esc_attr( $_POST['data_id'] );
		$bk_category = esc_attr( $_POST['bk_category'] );
		$bk_service = esc_attr( $_POST['bk_service'] );
		$subject = esc_attr( $_POST['subject'] );
		$username = esc_attr( $_POST['username'] );
		...
		if ($bk_user_to == $user_identity) {
			$status = 'approved';
		}

		$update_time = false;
		if (!empty($_POST['slottime'])) {
			$update_time = true;
			$slottime = esc_attr( $_POST['slottime'] );
			$booking_date = esc_attr( $_POST['booking_date'] );
			$timestamp = strtotime(esc_attr( $_POST['booking_date'] ));
		...
		$post_id = wp_update_post($appointment);
		...
		if ($update_time) {
			$appointment_meta['bk_slottime'] = $slottime;
			$appointment_meta['bk_booking_date'] = $booking_date;
			$appointment_meta['bk_timestamp'] = $timestamp;
		}
		...
		$json['data'] = docdirect_get_booking_step_five($user_to);
		$json['message'] = esc_html__('The boooking has been updated.','docdirect');	
		$json['type'] = 'success';
		$json['payment_type'] = 'local';
		echo json_encode($json);
		die;
	}
	add_action('wp_ajax_docdirect_update_booking','docdirect_update_booking');
	add_action('wp_ajax_nopriv_docdirect_update_booking', 'docdirect_update_booking');
}

if ( ! function_exists( 'docdirect_insert_booking' ) ) {
	function docdirect_insert_booking(){
		global $current_user;
		...
		//User Detail
		$currency = get_user_meta($user_from, 'currency', true);
			 
		//Price
		$services = get_user_meta($user_from, 'booking_services', true);
	...
	add_action('wp_ajax_docdirect_insert_booking','docdirect_insert_booking');
	add_action( 'wp_ajax_nopriv_docdirect_insert_booking', 'docdirect_insert_booking' );
}


// // Auto mailing for appoinment reminder
// // Scheduled Action Hook
// if (!function_exists('appoinment_reminder_send_mail')) {
// 	function appoinment_reminder_send_mail( ) {
// 		return wp_mail( 'test@uzmantavsiyeediyorum.com', 'Notification TEST 2', 'TEST 2', null );
// 		echo 'cron job';
// 	}
// 	add_action( 'appoinment_reminder_send_mail', 'appoinment_reminder_send_mail' );
// }

// // Custom Cron Recurrences
// if (!function_exists('appoinment_reminder_cron_job_recurrence')) {
// 	function appoinment_reminder_cron_job_recurrence( $schedules ) {
// 		$schedules['minutely'] = array(
// 			'display' => __( 'Each Minute', 'textdomain' ),
// 			'interval' => 60,
// 		);
// 		return $schedules;
// 	}
// 	add_filter( 'cron_schedules', 'appoinment_reminder_cron_job_recurrence' );
// }

// // Schedule Cron Job Event
// if (!function_exists('appoinment_reminder_cron_job')) {
// 	function appoinment_reminder_cron_job() {
// 		if ( ! wp_next_scheduled( 'appoinment_reminder_send_mail' ) ) {
// 			wp_schedule_event( current_time( 'timestamp' ), 'minutely', 'appoinment_reminder_send_mail' );
// 		}
// 	}
// 	add_action( 'wp', 'appoinment_reminder_cron_job' );
// }