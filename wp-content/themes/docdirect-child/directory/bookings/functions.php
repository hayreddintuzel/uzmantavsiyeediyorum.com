...
if ( ! function_exists( 'docdirect_prepare_booking_step_two' ) ) {
    function docdirect_prepare_booking_step_two($user_id, $slot_date, $post_id=''){
        if( !empty( $slot_date ) ){
        ...
        //Data
        ob_start();
        // pretty_print($todays_defaults);
        // pretty_print($appointments_array);
        ...
        <div class="tg-doctimeslot <?php echo sanitize_html_class( $slotClass );?>">
            <div class="tg-box">
                <div class="tg-radio">
                    <input <?php echo esc_attr( $slot_status );?> id="<?php echo esc_attr( $post_id . '_' . $key );?>" value="<?php echo esc_attr( $key );?>" type="radio" name="slottime">
                    <label for="<?php echo esc_attr( $post_id . '_' . $key );?>"><?php echo date_i18n($time_format,strtotime('2016-01-01 '.$time[0]) );?>&nbsp;-&nbsp;<?php echo date_i18n($time_format,strtotime('2016-01-01 '.$time[1]) );?></label>
                </div>
            </div>
        </div>
        <?php
...
if ( ! function_exists( 'docdirect_get_booking_step_two' ) ) {
    function docdirect_get_booking_step_two(){
        ...
        if (!empty($_POST['post_id'])) {
            ...
            $json['data']    = docdirect_prepare_booking_step_two($user_id, $slot_date, $post_id);
        } else {
            $json['data']    = docdirect_prepare_booking_step_two($user_id, $slot_date);
        }
        $json['type']    = 'success';
