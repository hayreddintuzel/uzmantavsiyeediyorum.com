...
if ( ! function_exists( 'tg_unwanted_remove_meta_box' ) ) {
    function tg_unwanted_remove_meta_box($post_type) {
        remove_meta_box('tagsdiv-directory_type', 'directory_type', 'normal');
        ...
        if( is_page_template('directory/user_search.php') ){
            ...
            if( !empty( $_GET['geo_location'] )
                ...
                if( !empty( $Latitude ) && !empty( $Longitude ) ){
                    ...
                    $user_query->query_orderby  = ' ORDER BY CAST(mt2.meta_value AS CHAR) DESC,distance ASC'; // additional sorting
            ...
            } else if(( !empty( $_COOKIE['geo_location'] )) || (!empty( $_GET['geo_location'])))
                /*if(
                isset( $_GET['sort_by'] )
                &&
                (( $_GET['sort_by'] === 'recent' ) || ( $_GET['sort_by'] === 'sp_articles' ) || ( $_GET['sort_by'] === 'title' ) || ( $_GET['sort_by'] === 'likes' )))*/
            {
                $geo_location = NULL;
                if( !empty( $_COOKIE['geo_location'] )) {
                    $geo_location	= explode('|',$_COOKIE['geo_location']);
                }
                else if(!empty( $_GET['geo_location'])) {
                    $geo_location	= $_GET['geo_location'];
                }
                ...
                if( !empty( $Latitude ) && !empty( $Longitude ) ){
                    ...
                    //$user_query->query_where .= ' distance '; // additional where clauses
                    $user_query->query_orderby  .= ' ,distance ASC'; // additional sorting
                    //$user_query->query_limit .= ''; // if you need to adjust paging
                }
            }
...
if (!function_exists('docdirect_search_filters')) {
    ...
    function docdirect_search_filters() {
        ...
        $dir_search_directory_type = fw_get_db_settings_option('dir_search_directory_type');
        ...
        $directory_type  	   = !empty( $_GET['directory_type'] ) ? $_GET['directory_type'] : '';
            ...
            <div class="doc-widget doc-widgetsearch">
                ...
                <div class="doc-widgetcontent">
                    <fieldset>
                        ...
                        <?php if( isset( $dir_search_directory_type ) && $dir_search_directory_type === 'enable' ){?>
                            <div class="form-group">
                                <div class="doc-select">
                                    <select name="directory_type" class="chosen-select">
                                        <option value=""><?php esc_attr_e('Select directory_type','docdirect');?></option>
                                        <?php docdirect_get_term_options($directory_type,'directory_type');?>
                                    </select>
                                </div>
                            </div>
                        <?php }?>
...
if (!function_exists('docdirect_get_filter_key')) {
    ...
    function docdirect_get_filter_key($key = ''){
        $filters	= array(
            ...
            'directory_type' => 'dd_directory_type',
...
if (!function_exists('docdirect_is_action_allow')) {
    function docdirect_is_action_allow(){
        ...
        if( isset( $verify_user ) && ( $verify_user === 'off' || $verify_user === '' )  ){
            $json['type'] 	 	 	 = 'error';
